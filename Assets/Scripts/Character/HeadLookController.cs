//Uses IK to have the Characters turn their heads to look at a specific target (i.e. and item or other character)
using UnityEngine;
...
public class HeadLookController : MonoBehaviour
{
    ...
    public MindsEye OwnerMind;
    public CharacterDetails TargetDetails;
    //public Transform targetTransform;
    //public Vector3 target = Vector3.zero;
    public float effect = 1;
    float effectLerp = 1.0f;
    ...
    {
        if(effectLerp < 1.0f)
            effectLerp += Time.smoothDeltaTime;
        ...
        float distBetweenTargets = Vector3.Distance(transform.position, TargetDetails.HeadTarget.position);

        float desiredEffect = 1.0f;

        if (distBetweenTargets <= minDistance)
            desiredEffect = 1.0f;
        else if (distBetweenTargets >= maxDistance)
            desiredEffect = 0.0f;
        else
            desiredEffect = Mathf.SmoothStep(1.0f, 0.0f, (distBetweenTargets - minDistance) / (maxDistance - minDistance));

        effect = Mathf.Lerp(effect, desiredEffect, effectLerp);
        //effect *= OwnerMind.MemoryGraph.GetCueOpinion(TargetDetails.CharCue);
        ...
        Vector3 targetPoint = TargetDetails.HeadTarget.position;

        if(flipDirection)
            targetPoint = (TargetDetails.HeadTarget.position - transform.position) * -1.0f + transform.position;
        ...
        foreach (BendingSegment segment in segments)
        {
            ...
            Vector3 lookDirWorld = (targetPoint - segment.lastTransform.position).normalized;
