...
public class MasterAudioInspector : Editor {
	...
	public override void OnInspectorGUI() {
		...
		if (sounds.showAdvancedSettings) {
			...
			var newIgnore = EditorGUILayout.Toggle("Ignore Time Scale", sounds.ignoreTimeScale);
			...
			if (newFast != sounds.enableFastResponse) {
                UndoHelper.RecordObjectPropertyForUndo(ref isDirty, sounds, "toggle Fast GUI Refresh");
		...
		if (sounds.playListExpanded) {
			...
			if (!plControllerInScene) {
				...
				for (var i = 0; i < pcs.Count; i++) {
					...
					GUI.contentColor = Color.green;
					GUILayout.TextField("V " + controller.playlistVolume.ToString("N2"), 6, EditorStyles.miniLabel);
					var newVol = GUILayout.HorizontalSlider(controller.playlistVolume, 0f, 1f, GUILayout.Width(74));
	...
	private void AddSongToPlaylist(MasterAudio.Playlist pList, AudioClip aClip) {
		...
		if (lastClip.clip == null && pList.bulkLocationMode == MasterAudio.AudioLocation.Clip) {
			mus = lastClip;
			mus.clip = aClip;
		} else if (string.IsNullOrEmpty(lastClip.resourceFileName) && pList.bulkLocationMode == MasterAudio.AudioLocation.ResourceFile) {
			mus = lastClip;
			var resourceFileName = DTGUIHelper.GetResourcePath(aClip);
			if (string.IsNullOrEmpty(resourceFileName)) {
				resourceFileName = aClip.name;
